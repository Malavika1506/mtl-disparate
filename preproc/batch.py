...
def get_feed_dicts(data_train_np, placeholders, batch_size, inst_length):
    ...
    if additionsamp != 0:
        realsamp += 1
    ids1 = choice(range(0, inst_length), inst_length, replace=False)  # sample without replacement so we get every sample once # -additionsamp
...
def batch_feed_dicts(data_train_np, batch_size, inst_length):
    data_train_batched = []
    realsamp = int(inst_length/batch_size)
    additionsamp = inst_length%batch_size
    if additionsamp != 0:
    ...
    for i in range(0, realsamp):
        ...
        if i != realsamp:
            ...
            ids_sup = ids[start:realsamp]
        for key, value in data_train_np.items():
            batch_i[key] = [data_train_np[key][ii] for ii in ids_sup]
